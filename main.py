...
KEYWORD = "日産"
EXCEL_FILE_PATH = "news_articles.xlsx" # 保存するExcelファイル名
...
def write_to_excel(articles: list[dict], sheet_name: str):
    """
    取得した記事リストをExcelファイルに追記する関数。
    - 既存のExcelファイルを読み込み、URLが重複しない記事のみを追記します。
    - ファイルやシートが存在しない場合は新規に作成します。
    """
    if not articles:
        print(f"⚠️ {sheet_name}シートに追記すべき新しいデータはありません。")
        return

    new_articles_df = pd.DataFrame(articles)
    
    # 既存のExcelファイルとシートの全データを読み込む
    all_sheets = {}
    if os.path.exists(EXCEL_FILE_PATH):
        try:
            ...
            print(f"⚠️ Excelファイルの読み込みに失敗しました: {e}")
            # 読み込みに失敗した場合は、バックアップとしてファイルをリネーム
            os.rename(EXCEL_FILE_PATH, f"{EXCEL_FILE_PATH}_{datetime.now().strftime('%Y%m%d%H%M%S')}.bak")

    # 追記対象のシートの既存データを取得
    if sheet_name in all_sheets:
        existing_df = all_sheets[sheet_name]
        # URLの重複をチェックし、新しい記事のみをフィルタリング
        existing_urls = set(existing_df['URL'])
        new_data_df = new_articles_df[~new_articles_df['URL'].isin(existing_urls)]
        # 既存データと新しいデータを結合
        combined_df = pd.concat([existing_df, new_data_df], ignore_index=True)
    else:
        # シートが存在しない場合は、取得したデータがそのままシートのデータになる
        new_data_df = new_articles_df
    ...
    if not new_data_df.empty:
        ...
        try:
            ...
            print(f"✅ {len(new_data_df)}件をExcelファイル '{EXCEL_FILE_PATH}' の '{sheet_name}' シートに追記しました。")
        except Exception as e:
            print(f"❌ Excelファイルへの書き込みに失敗しました: {e}")
    else:
        print(f"⚠️ {sheet_name}シートに追記すべき新しいデータはありませんでした（全て既存の記事）。")


if __name__ == "__main__":
    print("\n--- Google News ---")
    google_news_articles = get_google_news_with_selenium(KEYWORD)
    write_to_excel(google_news_articles, "Google")

    print("\n--- Yahoo! News ---")
    yahoo_news_articles = get_yahoo_news_with_selenium(KEYWORD)
    write_to_excel(yahoo_news_articles, "Yahoo")

    print("\n--- MSN News ---")
    msn_news_articles = get_msn_news_with_selenium(KEYWORD)
    write_to_excel(msn_news_articles, "MSN")
